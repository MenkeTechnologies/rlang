//! Language Server Protocol over stdio (`Rscript --lsp`).
//!
...
//! completion draw on the primitive corpus. No output ever reaches the terminal
//! — JSON-RPC on stdio only. Structure follows the sibling interpreters' `lsp.rs`.
...
pub fn run() -> Result<(), String> {
    ...
    let init_result = serde_json::json!({
        ...
        "serverInfo": { "name": "rlang", "version": env!("CARGO_PKG_VERSION") },
    });
    conn.sender
...
fn hover() -> Hover {
    Hover {
        contents: HoverContents::Markup(MarkupContent {
            ...
            value: "**rlang** — R on the fusevm bytecode VM + Cranelift JIT.".to_string(),
        }),
        range: None,
    }
}
...
/// Parse the whole document; a syntax error maps to a single diagnostic on the
/// line named in its `line N:` prefix.
...
mod tests {
    ...
    #[test]
    fn diagnostics_report_the_failing_line() {
        let d = compute_diagnostics("x <- 1\ny <- (");
        assert_eq!(d.len(), 1);
    ...
    fn valid_source_has_no_diagnostics() {
        assert!(compute_diagnostics("f <- function(x) x + 1\nf(2)").is_empty());
    }
}
