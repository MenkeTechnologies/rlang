//! Debug Adapter Protocol over stdio (`Rscript --dap`).
//!
//! A minimal, spec-compliant adapter: it completes the initialize/launch
//! handshake, runs the launched `.R` program to completion while streaming its
...
//!
//! Program stdout is captured through a `pipe` + `dup2` on fd 1 so `print`/`cat`
//! land in `output` events instead of corrupting the JSON channel on fd 1.

use serde_json::{json, Value as J};
use std::io::{Read, Write};

/// Entry point for `Rscript --dap`.
