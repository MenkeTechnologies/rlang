//! rkyv-backed bytecode cache for compiled R scripts (mirrors
//! zshrs/rubylang/vimlrs). Versioned from day one so an `.R` file that compiled
//! once never breaks on a later run.
//!
//! Layout: a single shard at `~/.rlang/scripts.rkyv`. The *outer* container is a
//! zero-copy rkyv archive (`Shard`), validated on load; each *inner* entry blob
//! is a bincode-encoded `CProg` (the compiled `fusevm::Chunk`s), because
...
fn shard_path() -> Option<PathBuf> {
    let dir = dirs::home_dir()?.join(".rlang");
...
mod tests {
    ...
    #[test]
    fn key_is_stable_and_source_sensitive() {
        assert_eq!(key_for("x <- 1"), key_for("x <- 1"));
        assert_ne!(key_for("x <- 1"), key_for("x <- 2"));
    }
}
