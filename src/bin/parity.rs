//! `parity` — differential harness: rlang vs. the reference `Rscript`.
//!
//! Development tool, not part of the runtime. It runs every snippet in
//! `tests/data/parity_corpus.R` (separated by a `#==#` line) through both the
//! system `Rscript` (the oracle) and rlang in-process, and compares stdout.
//!
//! * default: print a parity report (parity / gap / panic counts, and each
//!   gap's expected-vs-got). Needs `Rscript` on PATH.
//! * `--freeze`: capture the oracle output for every snippet into
//!   `tests/data/parity_expected.txt`, which the CI-safe `tests/parity.rs`
//!   replays with no `Rscript` installed.
//!
//! Errors compare loosely: if the oracle exits non-zero and rlang also errors,
...
const SEP: &str = "#==#";
const CORPUS: &str = "tests/data/parity_corpus.R";
...
/// Freeze the reference `Rscript` stdout for every `examples/*.rb` into
...
fn freeze_examples() {
    ...
    for name in &names {
        ...
        if !out.status.success() {
            eprintln!(
                "parity: reference Rscript rejected {path}:\n{}",
                String::from_utf8_lossy(&out.stderr)
            );
            std::process::exit(1);
        }
        let stem = name.strip_suffix(".R").unwrap();
...
/// Run a snippet through the reference `Rscript`; on a non-zero exit, return a
/// normalized error marker so both sides can agree "this is rejected".
fn oracle(snippet: &str) -> String {
    match Command::new("Rscript").arg("-e").arg(snippet).output() {
...
/// Evaluate a snippet in rlang, capturing stdout and turning a runtime error or
/// panic into `Err`.
fn rlang_run(snippet: &str) -> Result<String, String> {
    let snippet = snippet.to_string();
    let res = std::panic::catch_unwind(move || capture_stdout(|| rlang::eval_str(&snippet)));
    match res {
        Ok((Ok(_), out)) => Ok(out),
