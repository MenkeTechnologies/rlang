...
//!
//! `Rscript --tiers script.R` prints the report.
//!
//! One rlang-specific caveat the report cannot state for itself: `host::run_chunk`
//! deliberately does not arm fusevm's tracing tier, because R lowers nearly
//! every operation — element fetch, comparison, `%%`, indexing — to a builtin
//! call the tracer refuses, so the tier never produced native code while its
//! per-backward-branch cache probe cost ~12% of a scalar loop. `traced` is
//! therefore structurally `false` here, and the report's value is the
//! `block-ineligible ops` list: it names the builtins that would have to become
//! native ops before arming the tier could pay for itself.
...
pub struct ChunkTiers {
    /// Which chunk this is — `main` for a whole R program.
    pub name: String,
...
/// has gone round enough times to be recorded. rlang writes program output
...
///
/// An R program is not one chunk: the top level and every closure body lower to
/// their own chunk, and a hot loop usually lives in a closure, so reporting
/// only `main` would answer the wrong question.
fn program_chunks(prog: &crate::compiler::Program) -> Vec<(String, Chunk)> {
    ...
    for (i, c) in prog.closures.iter().enumerate() {
        out.push((format!("closure #{i} ({})", c.params.join(", ")), c.chunk.clone()));
    }
    out
}
...
const PROGRAM: &str = "f <- function(n) {\n  t <- 0\n  i <- 0\n  while (i < n) { t <- t + i; i <- i + 1 }\n  t\n}\nf(200000)\n";
