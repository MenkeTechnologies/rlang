//! CI-safe replay of the example programs. Every `examples/*.R` is run through
//! the built `Rscript` binary; the test asserts it exits successfully (the scripts
...
//! reference `Rscript` produced, frozen by `cargo run --bin parity -- --freeze-examples`.
//!
//! This needs no R installed, so CI runs it. A regression that diverges
//! rlang from the reference — or breaks one of the in-script assertions —
...
fn examples_match_reference_r() {
    ...
    let mut scripts: Vec<_> = std::fs::read_dir(&examples_dir)
        ...
        .filter(|p| p.extension().map(|x| x == "R").unwrap_or(false))
        .collect();
    scripts.sort();
    assert!(
        !scripts.is_empty(),
